...
function build_le_stats_page() {
	...
	if ( empty( $per_page) || $per_page < 1 ) $per_page = $screen->get_option( 'stats_per_page', 'default' );
	...
	if ( isset($_GET['action']) ) {
		...
		switch($_GET['action']) {
			case "trash":
				...
				$handle = wpdbQuery("DELETE FROM $table WHERE id = '$id'", "query");
				$msg = "This item has been removed.";
			break;
		}
		if ($handle) {
			add_settings_error(
...
<div class="wrap">
	...
	<div id="stats-wrapper">
		...
		<table id="signups">
			...
			$rowsperpage = (int) $per_page > 0 ? (int) $per_page : 10;
			...
			<tr>
				<td>
					<strong><?php echo $result->id; ?></strong>
					<div class="row-actions">
						<span class="trash">
							<a href="?page=lefx_stats&amp;id=<?php echo $result->id; ?>&amp;action=trash" class="remove">Delete</a>
						</span>
					</div>
				</td>
				<td style="white-space:nowrap;"><?php echo $result->time; ?></td>
			...
			<tr>
				<td class="tablenav bottom" colspan="<?php echo (7 + count($custom_fields));?>">
					<ul class="pagination tablenav-pages">
						<li><span class="displaying-num"><?php echo $totalcount?> items</span></li>
						<?php 
						...
						if ($currentpage == 1) {
							echo '<li><a class="first-page disabled" title="Go to the first page" href="#">&laquo;</a></li>';
							echo '<li><a class="prev-page disabled" title="Go to the previous page" href="#">&lsaquo;</a></li>';
						}
						
						if ($currentpage > 1) {	   
						   echo "<li><a title='Go to the first page' href='?page=lefx_stats&order=$order&ad=$ad&currentpage=1'>&laquo;</a></li>";	   
						   $prevpage = $currentpage - 1;
						   echo "<li><a title='Go to the previous page' href='?page=lefx_stats&order=$order&ad=$ad&currentpage=$prevpage' class='prev'>&lsaquo;</a></li>";
						}
						...
						if ($currentpage != $totalpages) {
						   ...
						   echo "<li><a title='Go to the last page' href='?page=lefx_stats&order=$order&ad=$ad&currentpage=$nextpage' class='next'>&rsaquo;</a></li>";
						   echo "<li><a title='Go to the next page'  href='?page=lefx_stats&order=$order&ad=$ad&currentpage=$totalpages'>&raquo;</a></li>";
						} 

						if ($currentpage == $totalpages) {
							echo '<li><a class="first-page disabled" title="Go to the last page" href="#">&raquo;</a></li>';
							echo '<li><a class="prev-page disabled" title="Go to the next page" href="#">&rsaquo;</a></li>';
						}
	...
	<div class="wrap">
		...
		<div id="le-export">
			...
			<p>Use this option if you'd like to save your stats data to your computer as an Excel-friendly file.</p>
		 	...
		 	<form method="post" action="<?php echo get_template_directory_uri(); ?>/functions/create-csv.php" id="csvform">
