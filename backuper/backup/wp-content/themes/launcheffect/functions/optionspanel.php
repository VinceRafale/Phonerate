...
function register_fields($optionspanel_array, $optionspanel_name) {
	...
	$validate_setting = 'validate_setting_' . $optionspanel_name;
...
function lefx_form($optionspanel_name, $optionspanel_array) {
	lefx_fields($optionspanel_name, $optionspanel_array);
}
...
function lefx_fields($optionspanel_name, $optionspanel_array) {
	...
	<script type="text/javascript">
		jQuery(document).ready(function($){
			formfield = null;
			$('.media-upload a[href="#upload"]').click(function() {
				if ( $(this).parents('.premium-section').length > 0) return false;
				formfield = $(this).parent().prev('input').attr('name');
			...
			window.send_to_editor = function(html) {
				...
				$('.media-upload input[name="'+formfield+'"]').val(imgurl);
				tb_remove();
			}
				...
				<?php foreach ($value as $subsection): ?>
					...
					<?php foreach ($subsection as $op): ?>
						...
						<?php
							...
							echo "<select name=\"$optionname\" ".($sel_off?'disabled':'').">";
							...
							foreach ($selectarray as $option) {
								...
								echo "<option class=\"$nospace\" ".($sel_sel?'selected="selected"':'').">$option</option>";
							}
							...
							echo '</select>';
								...
								if($op['subtype'] == 'webfont'):
									...
									foreach($selectarray as $selectarray) {
										...
										$nospace = str_replace(' ','',$firstfive);
										echo '<li class="' . $nospace . '"><img src="' . get_template_directory_uri() . '/functions/im/webfont-previews/' . $nospace . '.png" alt="" /></li>';
						...
						<?php
							...
							// TEXT SMALL ///////////////////////
							if($op['type'] == 'text-small'): ?>

							<input name="<?php echo $op['option_name']; ?>" type='text' value="<?php echo get_option($op['option_name']); ?>" <?php if(($op['premium'] == 'section' && lefx_version() == 'free') || ($op['premium'] == 'item' && lefx_version() == 'free')) { echo 'disabled'; } ?> class="small" />
						...
						?>
							...
							<?php
							$opt_disable = get_option($op['option_disable']);
							if($opt_disable == true ){ $checked = "checked=\"checked\""; } else { $checked = "";} ?>
							...
							<div class="clearfix"></div>
							<br /><?php if($options) { $logopreview = "{$options[$op['option_name']]}"; if($logopreview) { echo "<div class=\"le-preview\"><img src='$logopreview' class=\"le-logopreview\" /></div>"; } }?>
							<div class="clearfix"></div>
						...
						?>
							...
							<input name="<?php echo $op['option_name']; ?>" type="text" class="customfield" value="<?php if (get_option($op['option_name']) != "") { echo stripslashes(get_option($op['option_name'])); } else { echo date("m/d/Y"); } ?>" <?php if(($op['premium'] == 'section' && lefx_version() == 'free') || ($op['premium'] == 'item' && lefx_version() == 'free')) { echo 'disabled'; } ?> />-->		<div class="custom_field_type">
						...
						<?php
							// Product Options //////////////////////////////
							if($op['type'] == 'prod-opt-name'):
						?>
							<input name="<?php echo $op['option_name']; ?>" type='text' value="<?php echo get_option($op['option_name']); ?>" <?php if(($op['premium'] == 'section' && lefx_version() == 'free') || ($op['premium'] == 'item' && lefx_version() == 'free')) { echo 'disabled'; } ?> />
						<?php endif; ?>
						<?php
							// Product Options //////////////////////////////
							if($op['type'] == 'prod-opt-price'):
						?>
							<input name="<?php echo $op['option_name']; ?>" type='text' value="<?php echo get_option($op['option_name']); ?>" <?php if(($op['premium'] == 'section' && lefx_version() == 'free') || ($op['premium'] == 'item' && lefx_version() == 'free')) { echo 'disabled'; } ?> />
						<?php endif; ?>
	...
	<?php if(isset($can_chimp_sync)): ?>
		<?php if($can_chimp_sync && get_option('lefx_mclistid') != '' && get_option('lefx_mcapikey') != ''): ?>
		...
		<div id="chimpSync">
			...
			?>
			<?php if (in_array(get_option('lefx_mclistid'), $chimplists, true)): ?>
	...
	<?php if(isset($can_aweber_sync)): ?>
		<?php if($can_aweber_sync && get_option('lefx_aweberlistid') != '' && get_option('lefx_aweberconsumerkey') != '' && get_option('lefx_aweberconsumersecret') != ''):
